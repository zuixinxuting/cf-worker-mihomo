...
`+E:b.comment=E,A.comment=A.comment.substring(E.length+1)}}if(!r&&!x&&!A.found){let E=v?s(t,v,A,i):e(t,A.end,x,null,A,i);c.items.push(E),u=E.range[2],us(v)&&i(E.range,"BLOCK_IN_FLOW",cs)}else{t.atKey=!0;let E=A.end,b=k?s(t,k,A,i):e(t,E,S,null,A,i);us(k)&&i(b.range,"BLOCK_IN_FLOW",cs),t.atKey=!1;let T=ie(x??[],{flow:a,indicator:"map-value-ind",next:v,offset:b.range[2],onError:i,parentIndent:n.indent,startOnNewline:!1});if(T.found){if(!r&&!A.found&&t.options.strict){if(x)for(let M of x){if(M===T.found)break;if(M.type==="newline"){i(M,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line");break}}A.start<T.found.offset-1024&&i(T.found,"KEY_OVER_1024_CHARS","The : indicator must be at most 1024 chars after the start of an implicit flow sequence key")}}else v&&("source"in v&&v.source&&v.source[0]===":"?i(v,"MISSING_CHAR",`Missing space after : in ${a}`):i(T.start,"MISSING_CHAR",`Missing , or : between ${a} items`));let fe=v?s(t,v,T,i):T.found?e(t,T.end,x,null,T,i):null;fe?us(v)&&i(fe.range,"BLOCK_IN_FLOW",cs):T.comment&&(b.comment?b.comment+=`
`+T.comment:b.comment=T.comment);let xe=new C(b,fe);if(t.options.keepSourceTokens&&(xe.srcToken=w),r){let M=c;$t(t,M.items,b)&&i(E,"DUPLICATE_KEY","Map keys must be unique"),M.items.push(xe)}else{let M=new I(t.schema);M.flow=!0,M.items.push(xe);let _s=(fe??b).range;M.range=[b.range[0],_s[1],_s[2]],c.items.push(M)}u=fe?fe.range[2]:T.end}}let f=r?"}":"]",[h,...g]=n.end,d=u;if(h&&h.source===f)d=h.offset+h.source.length;else{let m=a[0].toUpperCase()+a.substring(1),w=p?`${m} must end with a ${f}`:`${m} in block collection must be sufficiently indented and end with a ${f}`;i(u,p?"MISSING_CHAR":"BAD_INDENT",w),h&&h.source.length!==1&&g.unshift(h)}if(g.length>0){let m=oe(g,d,t.options.strict,i);m.comment&&(c.comment?c.comment+=`
