...
`)?V(t,e):t.includes(`
`)?`
`+V(t,e):(s.endsWith(" ")?"":" ")+t;var Wt="flow",ct="block",Fe="quoted";function Ue(s,e,t="flow",{indentAtStart:n,lineWidth:i=80,minContentWidth:o=20,onFold:r,onOverflow:a}={}){if(!i||i<0)return s;i<o&&(o=0);let l=Math.max(1+o,1+i-e.length);if(s.length<=l)return s;let c=[],d={},f=i-e.length;typeof n=="number"&&(n>i-Math.max(2,o)?c.push(0):f=i-n);let u,m,g=!1,p=-1,h=-1,w=-1;t===ct&&(p=Es(s,p,e.length),p!==-1&&(f=p+l));for(let k;k=s[p+=1];){if(t===Fe&&k==="\\"){switch(h=p,s[p+1]){case"x":p+=3;break;case"u":p+=5;break;case"U":p+=9;break;default:p+=1}w=p}if(k===`
`)t===ct&&(p=Es(s,p,e.length)),f=p+e.length+l,u=void 0;else{if(k===" "&&m&&m!==" "&&m!==`
`&&m!=="	"){let x=s[p+1];x&&x!==" "&&x!==`
...
${e}${s.slice(x+1,A)}`)}return S}function Es(s,e,t){let n=e,i=e+1,o=s[i];for(;o===" "||o==="	";)if(e<i+t)o=s[++e];else{do o=s[++e];while(o&&o!==`
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${c}`),v=!1,O=ut(n,!0);r!=="folded"&&e!==y.BLOCK_FOLDED&&(O.onOverflow=()=>{v=!0});let b=Ue(`${S}${A}${m}`,c,ct,O);if(!v)return`>${x}
...
`+v.comment:c.comment=v.comment),f=v.end;continue}!r&&t.options.strict&&me(k)&&i(k,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line")}if(h===0)v.comma&&i(v.comma,"UNEXPECTED_TOKEN",`Unexpected , in ${a}`);else if(v.comma||i(v.start,"MISSING_CHAR",`Missing , between ${a} items`),v.comment){let O="";e:for(let b of S)switch(b.type){case"comma":case"space":break;case"comment":O=b.source.substring(1);break e;default:break e}if(O){let b=c.items[c.items.length-1];_(b)&&(b=b.value??b.key),b.comment?b.comment+=`
`+O:b.comment=O,v.comment=v.comment.substring(O.length+1)}}if(!r&&!x&&!v.found){let O=A?s(t,A,v,i):e(t,v.end,x,null,v,i);c.items.push(O),f=O.range[2],ls(A)&&i(O.range,"BLOCK_IN_FLOW",as)}else{t.atKey=!0;let O=v.end,b=k?s(t,k,v,i):e(t,O,S,null,v,i);ls(k)&&i(b.range,"BLOCK_IN_FLOW",as),t.atKey=!1;let I=ie(x??[],{flow:a,indicator:"map-value-ind",next:A,offset:b.range[2],onError:i,parentIndent:n.indent,startOnNewline:!1});if(I.found){if(!r&&!v.found&&t.options.strict){if(x)for(let M of x){if(M===I.found)break;if(M.type==="newline"){i(M,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line");break}}v.start<I.found.offset-1024&&i(I.found,"KEY_OVER_1024_CHARS","The : indicator must be at most 1024 chars after the start of an implicit flow sequence key")}}else A&&("source"in A&&A.source&&A.source[0]===":"?i(A,"MISSING_CHAR",`Missing space after : in ${a}`):i(I.start,"MISSING_CHAR",`Missing , or : between ${a} items`));let fe=A?s(t,A,I,i):I.found?e(t,I.end,x,null,I,i):null;fe?ls(A)&&i(fe.range,"BLOCK_IN_FLOW",as):I.comment&&(b.comment?b.comment+=`
`+I.comment:b.comment=I.comment);let ke=new C(b,fe);if(t.options.keepSourceTokens&&(ke.srcToken=w),r){let M=c;It(t,M.items,b)&&i(O,"DUPLICATE_KEY","Map keys must be unique"),M.items.push(ke)}else{let M=new T(t.schema);M.flow=!0,M.items.push(ke);let Ss=(fe??b).range;M.range=[b.range[0],Ss[1],Ss[2]],c.items.push(M)}f=fe?fe.range[2]:I.end}}let u=r?"}":"]",[m,...g]=n.end,p=f;if(m&&m.source===u)p=m.offset+m.source.length;else{let h=a[0].toUpperCase()+a.substring(1),w=d?`${h} must end with a ${u}`:`${h} in block collection must be sufficiently indented and end with a ${u}`;i(f,d?"MISSING_CHAR":"BAD_INDENT",w),m&&m.source.length!==1&&g.unshift(m)}if(g.length>0){let h=oe(g,p,t.options.strict,i);h.comment&&(c.comment?c.comment+=`
