...
`+T.comment:b.comment=T.comment);let xe=new C(b,fe);if(t.options.keepSourceTokens&&(xe.srcToken=w),r){let M=c;$t(t,M.items,b)&&i(O,"DUPLICATE_KEY","Map keys must be unique"),M.items.push(xe)}else{let M=new I(t.schema);M.flow=!0,M.items.push(xe);let _s=(fe??b).range;M.range=[b.range[0],_s[1],_s[2]],c.items.push(M)}u=fe?fe.range[2]:T.end}}let f=r?"}":"]",[h,...g]=n.end,d=u;if(h&&h.source===f)d=h.offset+h.source.length;else{let m=a[0].toUpperCase()+a.substring(1),w=p?`${m} must end with a ${f}`:`${m} in block collection must be sufficiently indented and end with a ${f}`;i(u,p?"MISSING_CHAR":"BAD_INDENT",w),h&&h.source.length!==1&&g.unshift(h)}if(g.length>0){let m=ie(g,d,t.options.strict,i);m.comment&&(c.comment?c.comment+=`
