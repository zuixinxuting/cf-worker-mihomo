...
${c}${S}${t}${m}`}function Rn(s,e,t,n){let{type:i,value:o}=s,{actualString:r,implicitKey:a,indent:l,indentStep:c,inFlow:d}=e;if(a&&o.includes(`
...
`:case"\r":case"	":return!0;default:return!1}}var dn=new Set("0123456789ABCDEFabcdef"),ui=new Set("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-#;/?:@&=+$_.!~*'()"),Kt=new Set(",[]{}"),pi=new Set(` ,[]{}
...
</html>`}async function Ni(s,e){let t=new URL(s.url,`http://${s.headers.host}`),n={url:t,urls:t.searchParams.getAll("url"),userAgent:s.headers["user-agent"],rule:t.searchParams.get("template"),singbox:t.searchParams.get("singbox")==="true",mihomo:t.searchParams.get("mihomo")==="true",v2ray:t.searchParams.get("v2ray")==="true",udp:t.searchParams.get("udp")==="true",exclude_package:t.searchParams.get("ep")==="true",exclude_address:t.searchParams.get("ea")==="true",tailscale:t.searchParams.get("tailscale")==="true",IMG:process.env.IMG||kn,sub:process.env.SUB||Ss,Mihomo_default:process.env.MIHOMOTOP||An,singbox_1_11:process.env.SINGBOX_1_11||xn,singbox_1_12:process.env.SINGBOX_1_12||vn,singbox_1_12_alpha:process.env.SINGBOX_1_12_ALPHA||Nn,beian:process.env.BEIAN||En,beianurl:process.env.BEIANURL||On,configs:_n(process.env.MIHOMO,process.env.SINGBOX),modes:Cn(process.env.SUB||Ss,s.headers["user-agent"])};if(n.urls.length===1&&n.urls[0].includes(",")&&(n.urls=n.urls[0].split(",").map(i=>i.trim())),n.urls.length===0||n.urls[0]===""){let i=await In(n);e.setHeader("Content-Type","text/html; charset=utf-8"),e.statusCode=200,e.end(i);return}try{let i;n.singbox?i=await Tn(n):n.mihomo?i=await Ln(n):n.v2ray&&(e=await getv2ray_config(n));let o=i.headers,r=["transfer-encoding","content-length","content-encoding","connection"];for(let[a,l]of Object.entries(o))r.includes(a.toLowerCase())||e.setHeader(a,l);e.setHeader("Content-Type","application/json; charset=utf-8"),e.setHeader("Profile-web-page-url",t.origin),e.statusCode=i.status||200,e.end(i.data)}catch(i){e.statusCode=400,e.setHeader("Content-Type","application/json; charset=utf-8"),e.end(JSON.stringify({error:i.message}))}}export{Ni as default};
