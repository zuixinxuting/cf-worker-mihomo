...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${c}`),A=!1,E=pt(n,!0);o!=="folded"&&e!==y.BLOCK_FOLDED&&(E.onOverflow=()=>{A=!0});let b=Ve(`${S}${v}${h}`,c,ut,E);if(!A)return`>${x}
...
`+A.comment:c.comment=A.comment),f=A.end;continue}!o&&t.options.strict&&ge(k)&&i(k,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line")}if(m===0)A.comma&&i(A.comma,"UNEXPECTED_TOKEN",`Unexpected , in ${a}`);else if(A.comma||i(A.start,"MISSING_CHAR",`Missing , between ${a} items`),A.comment){let E="";e:for(let b of S)switch(b.type){case"comma":case"space":break;case"comment":E=b.source.substring(1);break e;default:break e}if(E){let b=c.items[c.items.length-1];_(b)&&(b=b.value??b.key),b.comment?b.comment+=`
`+E:b.comment=E,A.comment=A.comment.substring(E.length+1)}}if(!o&&!x&&!A.found){let E=v?s(t,v,A,i):e(t,A.end,x,null,A,i);c.items.push(E),f=E.range[2],fs(v)&&i(E.range,"BLOCK_IN_FLOW",cs)}else{t.atKey=!0;let E=A.end,b=k?s(t,k,A,i):e(t,E,S,null,A,i);fs(k)&&i(b.range,"BLOCK_IN_FLOW",cs),t.atKey=!1;let T=ne(x??[],{flow:a,indicator:"map-value-ind",next:v,offset:b.range[2],onError:i,parentIndent:n.indent,startOnNewline:!1});if(T.found){if(!o&&!A.found&&t.options.strict){if(x)for(let M of x){if(M===T.found)break;if(M.type==="newline"){i(M,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line");break}}A.start<T.found.offset-1024&&i(T.found,"KEY_OVER_1024_CHARS","The : indicator must be at most 1024 chars after the start of an implicit flow sequence key")}}else v&&("source"in v&&v.source&&v.source[0]===":"?i(v,"MISSING_CHAR",`Missing space after : in ${a}`):i(T.start,"MISSING_CHAR",`Missing , or : between ${a} items`));let ue=v?s(t,v,T,i):T.found?e(t,T.end,x,null,T,i):null;ue?fs(v)&&i(ue.range,"BLOCK_IN_FLOW",cs):T.comment&&(b.comment?b.comment+=`
`+T.comment:b.comment=T.comment);let xe=new C(b,ue);if(t.options.keepSourceTokens&&(xe.srcToken=w),o){let M=c;$t(t,M.items,b)&&i(E,"DUPLICATE_KEY","Map keys must be unique"),M.items.push(xe)}else{let M=new I(t.schema);M.flow=!0,M.items.push(xe);let As=(ue??b).range;M.range=[b.range[0],As[1],As[2]],c.items.push(M)}f=ue?ue.range[2]:T.end}}let u=o?"}":"]",[h,...g]=n.end,d=f;if(h&&h.source===u)d=h.offset+h.source.length;else{let m=a[0].toUpperCase()+a.substring(1),w=p?`${m} must end with a ${u}`:`${m} in block collection must be sufficiently indented and end with a ${u}`;i(f,p?"MISSING_CHAR":"BAD_INDENT",w),h&&h.source.length!==1&&g.unshift(h)}if(g.length>0){let m=ie(g,d,t.options.strict,i);m.comment&&(c.comment?c.comment+=`
...
${n}`:n;else if(i||e.directives.docStart||!r)e.commentBefore=n;else if(O(r)&&!r.flow&&r.items.length>0){let o=r.items[0];_(o)&&(o=o.key);let a=o.commentBefore;o.commentBefore=a?`${n}
