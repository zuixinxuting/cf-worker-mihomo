...
${t}`)+"'";return e.implicitKey?n:Ue(n,t,Ht,ut(e,!1))}function _e(s,e){let{singleQuote:t}=e.options,n;if(t===!1)n=Ge;else{let i=s.includes('"'),o=s.includes("'");i&&!o?n=Wt:o&&!i?n=Ge:n=t?Wt:Ge}return n(s,e)}var zt;try{zt=new RegExp(`(^|(?<!
))
+(?!
...
${c}${k}${t}${m}`}function Kn(s,e,t,n){let{type:i,value:o}=s,{actualString:r,implicitKey:a,indent:l,indentStep:c,inFlow:d}=e;if(a&&o.includes(`
...
`:case"\r":case"	":return!0;default:return!1}}var un=new Set("0123456789ABCDEFabcdef"),fi=new Set("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-#;/?:@&=+$_.!~*'()"),Kt=new Set(",[]{}"),ui=new Set(` ,[]{}
...
</html>    `}async function vi(s,e){let t=new URL(s.url,`http://${s.headers.host}`),n={url:t,urls:t.searchParams.getAll("url"),userAgent:s.headers["user-agent"],rule:t.searchParams.get("template"),singbox:t.searchParams.get("singbox")==="true",mihomo:t.searchParams.get("mihomo")==="true",udp:t.searchParams.get("udp")!=="false",exclude_package:t.searchParams.get("ep")==="true",exclude_address:t.searchParams.get("ea")==="true",IMG:process.env.IMG||kn,sub:process.env.SUB||Sn,Mihomo_default:process.env.MIHOMO||xn,singbox_1_11:process.env.SINGBOX_1_11||An,singbox_1_12:process.env.SINGBOX_1_12||vn,singbox_1_12_alpha:process.env.SINGBOX_1_12_ALPHA||Nn,beian:process.env.BEIAN||_n,beianurl:process.env.BEIANURL||Ln,configs:En()};if(n.urls.length===1&&n.urls[0].includes(",")&&(n.urls=n.urls[0].split(",").map(i=>i.trim())),n.urls.length===0||n.urls[0]===""){let i=await Tn(n);e.setHeader("Content-Type","text/html; charset=utf-8"),e.statusCode=200,e.end(i);return}try{let i;n.singbox?i=await Cn(n):i=await On(n);let o=i.headers,r=["transfer-encoding","content-length","content-encoding","connection"];for(let[a,l]of Object.entries(o))r.includes(a.toLowerCase())||e.setHeader(a,l);e.setHeader("Content-Type","application/json; charset=utf-8"),e.setHeader("Profile-web-page-url",t.origin),e.statusCode=i.status||200,e.end(i.data)}catch(i){e.statusCode=400,e.setHeader("Content-Type","application/json; charset=utf-8"),e.end(JSON.stringify({error:i.message}))}}export{vi as default};
