...
${e}${s.slice(x+1,A)}`)}return k}function Os(s,e,t){let n=e,i=e+1,o=s[i];for(;o===" "||o==="	";)if(e<i+t)o=s[++e];else{do o=s[++e];while(o&&o!==`
...
`+I.comment:b.comment=I.comment);let Se=new C(b,fe);if(t.options.keepSourceTokens&&(Se.srcToken=w),r){let M=c;It(t,M.items,b)&&i(O,"DUPLICATE_KEY","Map keys must be unique"),M.items.push(Se)}else{let M=new T(t.schema);M.flow=!0,M.items.push(Se);let Ss=(fe??b).range;M.range=[b.range[0],Ss[1],Ss[2]],c.items.push(M)}f=fe?fe.range[2]:I.end}}let u=r?"}":"]",[m,...g]=n.end,p=f;if(m&&m.source===u)p=m.offset+m.source.length;else{let h=a[0].toUpperCase()+a.substring(1),w=d?`${h} must end with a ${u}`:`${h} in block collection must be sufficiently indented and end with a ${u}`;i(f,d?"MISSING_CHAR":"BAD_INDENT",w),m&&m.source.length!==1&&g.unshift(m)}if(g.length>0){let h=oe(g,p,t.options.strict,i);h.comment&&(c.comment?c.comment+=`
...
`:case"\r":case"	":return!0;default:return!1}}var pn=new Set("0123456789ABCDEFabcdef"),fi=new Set("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-#;/?:@&=+$_.!~*'()"),Rt=new Set(",[]{}"),ui=new Set(` ,[]{}
